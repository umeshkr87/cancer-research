...
echo ""
echo "📁 VALIDATING INPUT FILES - VCF MODE"
echo "===================================="
...
if [ -f "$VCF_FILE" ]; then
    ...
    echo "✅ VCF file found: $FILE_SIZE, $ROW_COUNT variants"
    
    # Validate VCF structure
    if grep -q "##INFO=<ID=CSQ" "$VCF_FILE"; then
        echo "✅ CSQ header found in VCF"
        CSQ_FIELDS=$(grep "##INFO=<ID=CSQ" "$VCF_FILE" | grep -o "Format: [^\"]*" | cut -d: -f2- | tr '|' '\n' | wc -l)
        echo "✅ CSQ fields detected: $CSQ_FIELDS"
    else
        echo "❌ ERROR: No CSQ header found in VCF file"
        exit 1
    fi
    ...
    echo "💡 This file should be generated by VEP annotation pipeline"
    exit 1
fi
...
if [ -f "$ENHANCED_FILE" ]; then
    ...
    echo "✅ Enhanced CSV found: $FILE_SIZE, $((ROW_COUNT-1)) variants (for comparison)"
else
    echo "⚠️  Enhanced CSV not found (VCF-only analysis mode)"
fi
...
if [ ! -f "$AUDIT_SCRIPT" ]; then
    echo "❌ ERROR: Audit script not found. Please ensure comprehensive_column_audit.py is in:"
    echo "   $VALIDATION_DIR/"
    exit 1
else
...
if [ $ESTIMATED_MEMORY_MB -gt 25000 ]; then
    echo "⚠️  Large VCF file detected. Processing may take significant time."
fi
...
echo ""
echo "🚀 RUNNING COMPREHENSIVE DATA QUALITY AUDIT - VCF MODE"
echo "======================================================"
...
if [ "$AUDIT_STATUS" = "SUCCESS" ] && [ $RESULTS_GENERATED -eq 3 ]; then
    echo "✅ SUCCESS! VCF AUDIT COMPLETED!"
    echo ""
